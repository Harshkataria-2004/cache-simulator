...
static std::unique_ptr<CacheHierarchy> build_default_hierarchy(ReplacementPolicy pol)
{
    ...
    // L3: 512 KB, 16-way, 64B blocks (smaller so matrix overflows into it)
    h->add_level(std::make_unique<CacheLevel>(
        "L3", 512 * 1024, 16, 64, pol, WritePolicy::WRITE_BACK, 36));
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2)
    {
        ...
        if (pol_str == "fifo")
            pol = ReplacementPolicy::FIFO;
        else if (pol_str == "lfu")
            pol = ReplacementPolicy::LFU;
        else if (pol_str == "random")
            pol = ReplacementPolicy::RANDOM;
