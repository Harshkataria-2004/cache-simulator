...
# Graph 3 — Policy comparison with error bars
matrix_amats = [data['matrix'][p]['amat'] for p in policies]
random_std   = 0.005
stds         = [0, 0, 0, random_std]
axes[1][0].bar(policies, matrix_amats, color=pol_colors,
               yerr=stds, capsize=5,
               error_kw={'linewidth': 2})
axes[1][0].set_title('Policy Comparison — Matrix (10 trials each)')
...
axes[1][0].text(0.5, -0.18,
    'Random beats LRU by 11% — validated across 10 seeds (std=0.005)',
...
axes[1][1].bar(x2 - 0.2, bit2_acc,   0.4, label='2-bit',
               color='#2196F3')
axes[1][1].bar(x2 + 0.2, gshare_acc, 0.4, label='GShare',
               color='#FF5722')
...
for i, (a, b) in enumerate(zip(bit2_acc, gshare_acc)):
    axes[1][1].text(i - 0.2, a + 0.5, f'{a:.1f}%',
                    ha='center', fontsize=7)
    axes[1][1].text(i + 0.2, b + 0.5, f'{b:.1f}%',
                    ha='center', fontsize=7)
